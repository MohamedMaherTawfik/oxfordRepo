<x-panel>
    ...
    <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-6">
        <!-- Page Title -->
        <h1 class="text-3xl font-bold text-[#79131d] mb-8"> {{ __('teacher.create_certificate') }}</h1>

        <!-- Form Card -->
        <div class="max-w-2xl w-full bg-white p-6 rounded-lg shadow">
            <h2 class="text-xl font-semibold mb-4"> {{ __('teacher.assign_certificate_to_student') }}</h2>
            ...
            <form action="{{ route('admin.certificates.store', ['slug' => $slug, 'id' => Auth::user()->id]) }}"
                ...
                <div class="mb-4">
                    <label for="certificate_name" class="block text-sm font-medium text-gray-700">
                        {{ __('teacher.certificate_name') }}</label>
