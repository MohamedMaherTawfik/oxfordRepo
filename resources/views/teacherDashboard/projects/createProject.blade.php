    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
            <div class="text-center">
                <h2 class="mt-6 text-3xl font-extrabold text-gray-900">{{ __('teacher.create_new') }}</h2>
            </div>
            ...
            <form class="mt-8 space-y-6" action="{{ route('teacher.project.store', request('slug')) }}" method="POST"
                ...
                <div class="rounded-md shadow-sm space-y-4">
                    ...
                    <div>
                        <label for="status"
                            class="block text-sm font-medium text-gray-700">{{ __('teacher.status') }}</label>
                        <select id="status" name="status"
                            ...
                            <option value="in_progress">{{ __('teacher.in_progress') }}</option>
                            <option value="completed">{{ __('teacher.completed') }}</option>
                        </select>
                        @error('status')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
